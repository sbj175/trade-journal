#!/usr/bin/env python3
"""
Fix chain detection logic to handle multiple CLOSING orders that together close a position
"""
import sqlite3
...
def calculate_net_position_quantity(chain_members, conn):
    """
    Calculate the net position quantity for a chain
    Returns dict of position_key -> net_quantity
    """
    cursor = conn.cursor()
    position_quantities = {}
    ...
    for member in chain_members:
        ...
        for pos in positions:
            symbol, option_type, strike, expiration, quantity = pos
            position_key = (symbol.strip(), option_type, strike, expiration)
            
            if position_key not in position_quantities:
                position_quantities[position_key] = 0
            position_quantities[position_key] += quantity
    
    return position_quantities

def is_position_fully_closed(chain_members, conn):
    """
    Check if all positions in the chain are fully closed (net quantity = 0)
    """
    position_quantities = calculate_net_position_quantity(chain_members, conn)
    return all(abs(qty) < 0.001 for qty in position_quantities.values())

def rebuild_order_chains_with_partial_close_support():
    """Rebuild order chains with support for multiple CLOSING orders"""
    
    conn = sqlite3.connect('trade_journal.db')
    cursor = conn.cursor()
    
    try:
        print("Rebuilding order chains with partial close support...")
        ...
        for account, orders in by_account.items():
            ...
            for order in orders:
                ...
                # ENHANCED: Build chains by finding continuation orders (ROLLING or CLOSING)
                # Support multiple CLOSING orders for partial closes
                if not is_system_closed and order['order_type'] != 'CLOSING':
                    ...
                    while True:
                        ...
                        closing_orders_found = []
                        
                        # First pass: collect all CLOSING orders that match this position
                        for candidate in orders:
                            if (candidate['order_id'] not in processed_orders and
                                candidate['order_type'] == 'CLOSING' and
                                ...
                                closing_orders_found.append(candidate)
                        
                        # Add all matching CLOSING orders to the chain
                        for closing_order in closing_orders_found:
                            chain_members.append(closing_order)
                            processed_orders.add(closing_order['order_id'])
                            
                            # Update linked_order_id
                            cursor.execute("""
                                UPDATE orders SET linked_order_id = ?
                                WHERE order_id = ?
                            """, (current_order['order_id'], closing_order['order_id']))
                            
                            print(f"  Linked CLOSING order {closing_order['order_id']} to {current_order['order_id']}")
                        
                        # Check if position is fully closed
                        if closing_orders_found:
                            if is_position_fully_closed(chain_members, conn):
                                print(f"  Position fully closed after adding {len(closing_orders_found)} CLOSING orders")
                                found_continuation = False  # Stop - position is fully closed
                            else:
                                print(f"  Position partially closed, continue looking for more closings")
                                found_continuation = False  # For now, stop after first batch of closings
                        
                        # Look for ROLLING orders (original logic)
                        if not closing_orders_found:
                            for candidate in orders:
                                if (candidate['order_id'] not in processed_orders and
                                    ...
                                    print(f"  Linked ROLLING order {candidate['order_id']} to {current_order['order_id']}")
                ...
                if chain_members:
                    ...
                    # ENHANCED: Determine chain status based on position closure
                    if is_system_closed:
                        chain_status = 'CLOSED'
                    else:
                        # Check if all positions are fully closed
                        if is_position_fully_closed(chain_members, conn):
                            chain_status = 'CLOSED'
                        else:
                            chain_status = 'OPEN'
        ...
        conn.commit()
        print("\n✅ Order chains rebuilt with partial close support!")
        
        # Test our specific orders
        print("\n=== TESTING PARTIAL CLOSE ORDERS ===")
        cursor.execute('''
            ...
            WHERE ocm.order_id IN ("380367633", "381208759", "381209221")
        ...
        if test_results:
            ...
            if len(chain_ids) == 1:
                print(f"\\n🎉 SUCCESS: All three orders are now in the same chain!")
            else:
...
if __name__ == "__main__":
    rebuild_order_chains_with_partial_close_support()